...
import type { MarkOrderDeliveredCommand } from './mark-order-delivered.command';
...
export class MarkOrderDeliveredUseCase {
    ...
    async execute(command: MarkOrderDeliveredCommand): Promise<OrderDeliveryView> {
        ...
        if (order.status !== OrderStatus.DELIVERING) throw new DeliveryAccessDeniedError();

        const result = await this.orderDeliveryWriter.markDelivered(command.orderId, command.currentUserId);
        await this.courierDeliveryWriter.markOnline(courier.courierProfileId);
        await this.courierDeliveryWriter.incrementCompletedDeliveries(courier.courierProfileId);

        await this.eventPublisher.publishAll([
            new OrderDeliveredEvent(command.orderId, command.currentUserId, result.deliveredAt!),
        ]);
