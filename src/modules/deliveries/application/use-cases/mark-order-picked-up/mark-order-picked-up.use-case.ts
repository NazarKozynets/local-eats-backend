...
import type { MarkOrderPickedUpCommand } from './mark-order-picked-up.command';

@Injectable()
export class MarkOrderPickedUpUseCase {
    constructor(
    ...
    async execute(command: MarkOrderPickedUpCommand): Promise<OrderDeliveryView> {
        ...
        if (order.status !== OrderStatus.ASSIGNED_TO_COURIER) throw new DeliveryAccessDeniedError();

        const result = await this.orderDeliveryWriter.markPickedUp(command.orderId, command.currentUserId);

        await this.eventPublisher.publishAll([
            new OrderPickedUpEvent(command.orderId, command.currentUserId, result.pickedUpAt!),
        ]);
