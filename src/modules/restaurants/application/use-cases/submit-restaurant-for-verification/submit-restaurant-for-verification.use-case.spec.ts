import { SubmitRestaurantForVerificationUseCase } from './submit-restaurant-for-verification.use-case';
...
describe('SubmitRestaurantForVerificationUseCase', () => {
    ...
    it('throws InvalidRestaurantStatusTransitionError when restaurant is ACTIVE', async () => {
        staffRepository.findByRestaurantAndUser.mockResolvedValue(buildStaff());
    ...
    it('sets status to PENDING_VERIFICATION for DRAFT restaurant', async () => {
        const restaurant = buildRestaurant({ status: RestaurantStatus.DRAFT });
        staffRepository.findByRestaurantAndUser.mockResolvedValue(buildStaff());
        restaurantRepository.findById.mockResolvedValue(restaurant);

        await useCase.execute(command());

        expect(restaurant.status).toBe(RestaurantStatus.PENDING_VERIFICATION);
    });

    it('publishes RestaurantSubmittedForVerificationEvent on success', async () => {
        staffRepository.findByRestaurantAndUser.mockResolvedValue(buildStaff());
        restaurantRepository.findById.mockResolvedValue(
        ...
        expect(eventPublisher.publishAll).toHaveBeenCalledWith(
            expect.arrayContaining([expect.any(RestaurantSubmittedForVerificationEvent)]),
        );
    });
});
