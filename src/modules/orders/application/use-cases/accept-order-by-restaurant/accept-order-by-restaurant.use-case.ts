...
import type { AcceptOrderByRestaurantCommand } from './accept-order-by-restaurant.command';
...
export class AcceptOrderByRestaurantUseCase {
    ...
    async execute(command: AcceptOrderByRestaurantCommand): Promise<void> {
        ...
        order.acceptByRestaurant(command.restaurantComment);
        ...
        await this.orderRepository.saveWithHistory(order, {
            ...
            comment: command.restaurantComment,
        });

        await this.eventPublisher.publishAll([
            new OrderAcceptedByRestaurantEvent(
                order.id.value,
                order.publicCode,
